-- Allow repeat exposures: optional bypass of seen_tokens anti-join.
-- Frontend can toggle allow_repeat=true to re-show previously seen tokens.
...
as $$
  with base as (
    ...
    where (p_allow_repeat or st.token_id is null)
